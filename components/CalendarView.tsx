...
export function CalendarView({ isDarkMode, onClose }: CalendarViewProps) {
  ...
  // Memoized current month and year
  ...
  // Calculate Orthodox Easter (Coptic Orthodox uses same calculation)
  const calculateOrthodoxEaster = useCallback((year: number): Date => {
    ...
    // Julian calendar date to Gregorian (add 13 days for 20th-21st century)
  ...
  const getTypeInfo = useCallback((type: string): TypeInfo => {
    const baseStyles = {
      major: {
        label: 'Grote Feest',
    ...
    return baseStyles[type as keyof typeof baseStyles] || {
      label: 'Feest',
  ...
  // Calculate moveable feasts based on Easter
  const calculateMoveableFeasts = useCallback((easter: Date, year: number, month: number): CalendarEvent[] => {
    const events: CalendarEvent[] = [];
    const easterMonth = easter.getMonth();
    const easterYear = easter.getFullYear();

    // Helper to add event if in current month
    const addEventIfInMonth = (date: Date, feast: CopticFeast) => {
      if (date.getMonth() === month && date.getFullYear() === year) {
        events.push({ date, feast });
      }
    };
    ...
    addEventIfInMonth(greatLentStart, {
      id: 'fast-great-lent',
      name: 'Grote Vasten',
      type: 'fast',
    ...
    const palmSundayFeast = moveableFeasts.find(f => f.id === 'palm-sunday');
    if (palmSundayFeast) {
      addEventIfInMonth(palmSunday, palmSundayFeast);
    }

    // Holy Thursday: 3 dagen voor Pasen
    const holyThursday = new Date(easter);
    holyThursday.setDate(easter.getDate() - 3);
    const holyThursdayFeast = moveableFeasts.find(f => f.id === 'holy-thursday');
    if (holyThursdayFeast) {
      addEventIfInMonth(holyThursday, holyThursdayFeast);
    }

    // Good Friday: 2 dagen voor Pasen
    const goodFriday = new Date(easter);
    goodFriday.setDate(easter.getDate() - 2);
    const goodFridayFeast = moveableFeasts.find(f => f.id === 'good-friday');
    if (goodFridayFeast) {
      addEventIfInMonth(goodFriday, goodFridayFeast);
    }

    // Holy Saturday: 1 dag voor Pasen
    const holySaturday = new Date(easter);
    holySaturday.setDate(easter.getDate() - 1);
    const holySaturdayFeast = moveableFeasts.find(f => f.id === 'holy-saturday');
    if (holySaturdayFeast) {
      addEventIfInMonth(holySaturday, {
        ...holySaturdayFeast,
    ...
    // Easter Sunday
    addEventIfInMonth(easter, moveableFeasts.find(f => f.id === 'easter')!);

    // Thomas Sunday: 7 dagen na Pasen
    const thomasSunday = new Date(easter);
    thomasSunday.setDate(easter.getDate() + 7);
    const thomasSundayFeast = moveableFeasts.find(f => f.id === 'thomas-sunday');
    if (thomasSundayFeast) {
      addEventIfInMonth(thomasSunday, thomasSundayFeast);
    }

    // Ascension: 40 dagen na Pasen (altijd op donderdag)
    const ascension = new Date(easter);
    ascension.setDate(easter.getDate() + 39);
    const ascensionFeast = moveableFeasts.find(f => f.id === 'ascension');
    if (ascensionFeast) {
      addEventIfInMonth(ascension, ascensionFeast);
    }

    // Pentecost: 50 dagen na Pasen
    const pentecost = new Date(easter);
    pentecost.setDate(easter.getDate() + 49);
    const pentecostFeast = moveableFeasts.find(f => f.id === 'pentecost');
    if (pentecostFeast) {
      addEventIfInMonth(pentecost, pentecostFeast);
    }
    ...
    addEventIfInMonth(apostlesFastStart, {
      id: 'fast-apostles',
  ...
  const monthEvents = useMemo((): CalendarEvent[] => {
    ...
    // Fasts (excluding weekly fasts and duplicates)
    fasts.forEach(fast => {
    ...
    const moveableEvents = calculateMoveableFeasts(easter, currentYear, currentMonth);
    events.push(...moveableEvents);

    // Sort by date
    return events.sort((a, b) => a.date.getTime() - b.date.getTime());
  }, [currentYear, currentMonth, calculateOrthodoxEaster, calculateMoveableFeasts]);
  ...
  const renderEvent = (event: CalendarEvent, index: number) => {
    ...
    return (
      <div
        ...
        className={`p-4 rounded-lg border-2 ${typeInfo.color} transition-all hover:shadow-lg hover:scale-[1.02]`}
      >
        <div className="flex items-start justify-between gap-3 mb-2">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <div className={`text-center min-w-[60px] flex-shrink-0 ${isDarkMode ? 'bg-black/20' : 'bg-white/50'} rounded-lg p-2`}>
              <div className={`text-2xl font-bold ${isDarkMode ? 'text-amber-400' : 'text-amber-700'}`}>
                {event.date.getDate()}
              </div>
          ...
          <span className={`text-xs px-2 py-1 rounded-full whitespace-nowrap flex-shrink-0 ${isDarkMode ? 'bg-black/30' : 'bg-white/50'}`}>
            {typeInfo.label}
          </span>
        </div>
      </div>
          ...
          <div className="px-4 py-3">
            ...
            <div className="flex items-center justify-between">
              <Button
                ...
                className={`flex items-center ${isDarkMode ? 'text-gray-300 hover:bg-gray-700' : ''}`}
              ...
              <h2 className={`text-xl font-semibold ${isDarkMode ? 'text-amber-300' : 'text-amber-700'}`}>
          ...
          <div className="px-4 py-6">
            {monthEvents.length === 0 ? (
              <div className={`text-center py-12 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                ...
                <p className="text-lg mb-2">Geen feesten of gedenkdagen</p>
                <p className="text-sm opacity-70">deze maand</p>
              </div>
            ) : (
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm opacity-80">
                    ...
                    behalve tijdens Paastijd en bepaalde feestperiodes.
                  </p>
                </div>
              </div>
            </div>
